#!/usr/bin/env python3
"""
GraphModel_08_GridSearch.py

Corrected version of GraphModel_07. Key fix: affinity matrices from FINCHES
use negative values for attraction. Correct preprocessing pipeline is:
  1. Sign flip  (negative attraction -> positive similarity)
  2. Clip to zero  (repulsive pairs -> zero affinity, not negative)
  3. MinMax scale to [0, 1]  (required by LabelSpreading and HC)

Optional row-wise normalization (standard/robust) is applied on top of this
base preprocessing as an additional grid dimension.

Grid: base_prep x row_norm x k_graph x alpha x conf_threshold
Parallelized with joblib across 12 physical cores.

Usage: python GraphModel_08_GridSearch.py
"""
...
OUTPUT_DIR = Path("./results_gridsearch_08")
...
# ---------------------------------------------------------------------------
# Grid — tightened around GM07 winners, with corrected preprocessing
# ---------------------------------------------------------------------------
# Row-wise normalization applied AFTER base preprocessing (none = skip)
TUNE_ROW_NORMS       = ["none", "standard", "robust"]
TUNE_K_GRAPH         = [10, 15, 20, 25, 30]
...
def preprocess_affinity(A: np.ndarray, row_norm: str = "none") -> np.ndarray:
    """
    Correct preprocessing pipeline for FINCHES affinity matrices:
      1. Sign flip  — negative attraction becomes positive similarity
      2. Clip       — repulsive (still negative after flip) -> 0
      3. MinMax     — scale to [0, 1] globally
      4. Row norm   — optional additional row-wise normalization

    Steps 1-3 are always applied. Step 4 is controlled by row_norm.
    """
    # 1. Sign flip
    A = -A.copy()
    # 2. Clip repulsive pairs to zero
    A = np.clip(A, 0.0, None)
    np.fill_diagonal(A, 0.0)
    # 3. Global MinMax to [0, 1]
    vmax = A.max()
    if vmax > 0:
        A = A / vmax
    # 4. Optional row-wise normalization
    if row_norm == "standard":
        A = StandardScaler().fit_transform(A)
        A = np.clip(A, 0.0, None)
        vmax = A.max()
        if vmax > 0:
            A = A / vmax
    elif row_norm == "robust":
...
def load_affinity_matrix(path: Path):
    ...
    A = 0.5 * (A + A.T)   # symmetrize raw values before preprocessing
    np.fill_diagonal(A, 0.0)
    return idr_names, A
...
def _evaluate_combo(row_norm, k_graph, alpha, datasets):
    """
    Evaluate one (row_norm, k_graph, alpha) combo across all datasets.
    Returns records for each conf_threshold. Top-level for pickling.
    """
    f1s, accs, weights = [], [], []
    batch_probas = {}

    for batch, idr_names, A_raw, y in datasets:
        A_proc   = preprocess_affinity(A_raw, row_norm)
        A_sparse = build_knn_graph(A_proc, k_graph)
    ...
    for conf in TUNE_CONF_THRESHOLDS:
        conf_vals, pct_vals = [], []
        for batch, idr_names, A_raw, y_b in datasets:
            y_batch, max_proba = batch_probas[batch]
...
def run_grid_search(pairs):
    print("\n" + "="*60)
    print("Grid search — corrected preprocessing (flip+clip+minmax)")
    ...
    prop_grid  = list(itertools.product(TUNE_ROW_NORMS, TUNE_K_GRAPH, TUNE_ALPHAS))
    total_prop = len(prop_grid)
    print(f"\nRunning {total_prop} combos × {len(TUNE_CONF_THRESHOLDS)} conf thresholds "
          f"in parallel (N_PARALLEL_JOBS={N_PARALLEL_JOBS})...")

    nested_records = Parallel(n_jobs=N_PARALLEL_JOBS, prefer="processes", verbose=5)(
        delayed(_evaluate_combo)(row_norm, k_graph, alpha, datasets)
        for row_norm, k_graph, alpha in prop_grid
    )
...
def pick_best_params(results_df: pd.DataFrame):
    ...
    print("="*60)
    print(f"  row_norm       = {best['row_norm']}")
    ...
    return (best["row_norm"], int(best["k_graph"]),
...
def plot_grid_heatmaps(results_df: pd.DataFrame, out_dir: Path):
    for row_norm in TUNE_ROW_NORMS:
        sub = results_df[results_df["row_norm"] == row_norm]
        agg = (sub.groupby(["alpha", "k_graph"])
                  [["mean_macro_f1", "mean_conf_assigned"]]
                  .mean().reset_index())
        for metric, label, fname in [
            ("mean_macro_f1",      "Weighted mean macro-F1",        f"heatmap_{row_norm}_f1.png"),
            ("mean_conf_assigned", "Mean confidence of assignments", f"heatmap_{row_norm}_conf.png"),
        ]:
            try:
                ...
                ax.set_title(f"row_norm={row_norm} — {label}\n(averaged over conf_threshold)")
    ...
    # Confidence threshold tradeoff for best combo
    best_row   = results_df.loc[results_df["mean_macro_f1"].idxmax()]
    best_norm  = best_row["row_norm"]
    best_k     = best_row["k_graph"]
    best_alpha = best_row["alpha"]
    sub = results_df[
        (results_df["row_norm"] == best_norm) &
...
def plot_normalization_comparison(results_df: pd.DataFrame, out_dir: Path):
    best_per_norm = (results_df.groupby("row_norm")["mean_macro_f1"]
                     .max().reset_index()
                     .sort_values("mean_macro_f1", ascending=False))
    fig, ax = plt.subplots(figsize=(7, 4))
    bars = ax.bar(best_per_norm["row_norm"], best_per_norm["mean_macro_f1"],
                  color="steelblue", alpha=0.85)
    for bar, v in zip(bars, best_per_norm["mean_macro_f1"]):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.002,
                f"{v:.3f}", ha="center", va="bottom", fontsize=9)
    ax.set_xlabel("Row normalization (applied after flip+clip+minmax)")
    ax.set_ylabel("Best weighted macro-F1")
    ax.set_title("Best LOO macro-F1 per row normalization method\n"
                 "(base preprocessing: sign flip + clip + minmax always applied)")
...
def plot_accuracy_summary(batch_loo, global_f1, global_acc, out_dir):
    ...
    ax.axvspan(len(batches) - 0.5, len(all_labels) - 0.5, alpha=0.08, color="grey")
    ax.set_xticks(x); ax.set_xticklabels(all_labels, rotation=30, ha="right")
    ax.set_ylabel("Score"); ax.set_ylim(0, 1.1)
    ax.set_title("LOO evaluation — GM08 best params\n"
                 "(preprocessing: sign flip + clip + minmax)")
...
def run_full(pairs, row_norm, k_graph, alpha, conf_threshold):
    print("\n" + "="*60)
    print("Full run with best params")
    print(f"  row_norm={row_norm}, k_graph={k_graph}, alpha={alpha}, conf={conf_threshold}")
    print("="*60)
    ...
    for aff_path, label_path, batch in pairs:
        ...
        ari = plot_hc_vs_ls(A_sparse, results_df,
                            OUTPUT_DIR / f"{batch}_hc_vs_ls.png", batch_name=batch)
    ...
    if loo_y_true_all:
        plot_confusion_matrix(np.array(loo_y_true_all), np.array(loo_y_pred_all),
                              ...
                              title="Global LOO confusion — GM08")
...
def main():
    ...
    # Quick sanity check on preprocessing
    idr_names, A_raw = load_affinity_matrix(pairs[0][0])
    A_check = preprocess_affinity(A_raw, "none")
    ...
    row_norm, k_graph, alpha, conf_threshold = pick_best_params(results_df)
    run_full(pairs, row_norm, k_graph, alpha, conf_threshold)

    print(f"\nAll outputs saved to {OUTPUT_DIR}/")
    print("\nTo generate post-run analysis figures run:")
    print(f"  python analysis_results.py --results {OUTPUT_DIR}/all_results.csv "
          f"--output results_analysis_08")
