#!/usr/bin/env python3
"""
GraphModel_07_GridSearch.py

Exhaustive hyperparameter grid search over:
  - normalization  : none | minmax | standard | robust (applied after sign flip)
  - k_graph        : KNN sparsification neighbours
  - alpha          : LabelSpreading clamping factor
  - confidence_thr : threshold below which a prediction is marked unknown

Only runs on the three priority batches (50_99, 100_150, 151_200) for speed.

Optimises on weighted macro-F1 (LOO). Also tracks accuracy and mean
confidence of assigned labels (higher confidence = fewer but more reliable
assignments).

Results saved to OUTPUT_DIR/grid_search_results.csv and a set of heatmap
figures. Best params are then used for a full run on the priority batches,
saving all_results.csv and per-batch plots.

Usage: python GraphModel_07_GridSearch.py
"""
...
OUTPUT_DIR = Path("./results_gridsearch")

# Only these three batches are used for tuning and the final run
...
# After grid search, use these defaults if tuning finds nothing better
DEFAULT_NORM   = "none"
DEFAULT_K      = 15
DEFAULT_ALPHA  = 0.3
DEFAULT_CONF   = 0.6
...
def normalize_affinity(A: np.ndarray, method: str) -> np.ndarray:
    """
    Apply normalization to the affinity matrix (after sign flip).

    none     — raw negated FINCHES scores, no further scaling
    minmax   — per-row MinMax to [0, 1]
    standard — per-row zero-mean unit-variance (StandardScaler)
    robust   — per-row median/IQR scaling (RobustScaler, outlier-resistant)

    All variants zero the diagonal after scaling.
    """
    if method == "none":
        return A
...
def build_knn_graph(A: np.ndarray, k: int) -> np.ndarray:
    """
    Keep only the top-k neighbours per row, then symmetrize (union).
    Zeroes the diagonal.
    """
    n = A.shape[0]
    k = min(k, n - 1)
...
def load_affinity_matrix(path: Path):
    ...
    # Sign flip: FINCHES negative = attraction → we want high = similar
    A = -A
...
def find_pairs():
    # Search recursively but deduplicate by stem so data/data/ nesting
    # doesn't produce duplicate entries for the same batch.
    seen  = set()
    pairs = []
...
def loo_accuracy(A, y, alpha, n_neighbors,
    ...
    """
    Repeated random holdout. Returns (mean_f1, mean_acc, y_true_all, y_pred_all).
    Note: confidence threshold is NOT applied during LOO — we evaluate raw
    predictions so the metric is independent of the threshold parameter.
    This lets us separate propagation quality from assignment strictness.
    """
...
def run_grid_search(pairs):
    """
    Full grid search over normalization x k_graph x alpha x conf_threshold.

    LOO F1/acc are computed on the propagation params (norm, k_graph, alpha)
    independently of conf_threshold — the threshold only affects how many
    IDRs get a final label, not the propagation quality itself.

    For each (norm, k_graph, alpha) combo we run LOO once, then for each
    conf_threshold we compute:
      - mean_conf_assigned : mean max_proba of IDRs that pass the threshold
      - pct_assigned       : % of unlabeled IDRs that get a label

    This gives a full picture: F1 tells you propagation quality, mean_conf
    and pct_assigned tell you the quality/quantity tradeoff.
    """
    print("\n" + "="*60)
    print("Grid search — normalization x k_graph x alpha x conf_threshold")
    print(f"Batches: {[b for _,_,b in pairs]}")
    print("="*60)

    # Pre-load raw matrices and labels
    datasets = []
    ...
    # Phase 1: evaluate (norm, k_graph, alpha) combos via LOO
    prop_grid = list(itertools.product(TUNE_NORMALIZATIONS, TUNE_K_GRAPH, TUNE_ALPHAS))
    total_prop = len(prop_grid)
    prop_records = []

    print(f"\nPhase 1: {total_prop} propagation combos × {len(datasets)} batches")
    for idx, (norm, k_graph, alpha) in enumerate(prop_grid):
        f1s, accs, weights = [], [], []
        # Also store full-run proba for phase 2
        batch_probas = {}

        for batch, idr_names, A_raw, y in datasets:
            A_norm   = normalize_affinity(A_raw, norm)
            A_sparse = build_knn_graph(A_norm, k_graph)
            k_spread = min(k_graph, 20)   # k_spread <= k_graph
            ...
            # Full run to get probas for threshold analysis
            _, pred_labels, max_proba = run_label_spreading(A_sparse, y, alpha, k_spread)
        ...
        print(f"  [{idx+1}/{total_prop}] norm={norm}, k={k_graph}, alpha={alpha}"
              f"  ->  F1={mean_f1:.3f}  acc={mean_acc:.3f}")

        # Phase 2: for each conf_threshold, compute assignment stats
    ...
    results_df = pd.DataFrame(prop_records)
...
# ---------------------------------------------------------------------------
# Pick best params and report
# ---------------------------------------------------------------------------

def pick_best_params(results_df: pd.DataFrame):
    """
    Primary sort: mean_macro_f1 descending.
    Tie-break: mean_conf_assigned descending (prefer confident assignments).
    """
...
# ---------------------------------------------------------------------------
# Plotting — grid search summary figures
# ---------------------------------------------------------------------------

def plot_grid_heatmaps(results_df: pd.DataFrame, out_dir: Path):
    """
    For each normalization method, plot a heatmap of mean_macro_f1
    with axes alpha (rows) x k_graph (cols), averaged over conf_threshold.
    Also plot a separate heatmap for mean_conf_assigned.
    """
    for norm in TUNE_NORMALIZATIONS:
        sub = results_df[results_df["norm"] == norm]
        # Average over conf_threshold for the propagation quality heatmap
    ...
    # Confidence threshold effect: for best (norm, k, alpha), show F1 vs conf
...
def plot_normalization_comparison(results_df: pd.DataFrame, out_dir: Path):
    ...
    ax.set_title("Best LOO macro-F1 per normalization method\n(maximized over k_graph, alpha, conf_threshold)")
    ax.grid(axis="y", linestyle="--", alpha=0.4)
    plt.tight_layout()
    plt.savefig(out_dir / "normalization_comparison.png", dpi=150)
    plt.close()
...
def run_full(pairs, norm, k_graph, alpha, conf_threshold):
    """
    Run LabelSpreading on all priority batches with the best params.
    Saves all_results.csv and per-batch plots.
    """
    print("\n" + "="*60)
    ...
    for aff_path, label_path, batch in pairs:
        ...
        # ARI
        ari = plot_hc_vs_ls(A_sparse, results_df,
    ...
    if loo_y_true_all:
        plot_confusion_matrix(np.array(loo_y_true_all), np.array(loo_y_pred_all),
                              ...
                              title="Global LOO confusion — grid search best params")
...
def plot_accuracy_summary(batch_loo, global_f1, global_acc, out_dir):
    ...
    ax.set_title("LOO evaluation — grid search best params")
...
def main():
    ...
    print(f"  python analysis_results.py --results {OUTPUT_DIR}/all_results.csv "
          f"--output results_analysis_gridsearch")
